<template>
    <div class="section" v-if="wallpapers">
        <p class="headerText">{{translation('TRENDING Wallpapers')}}</p>
...
export default {
    ...
    async mounted(){
        this.wallpapers = (await axios.get('https://taku.moe:2087/wallpapers/trending/20')).data.wallpapers; 
    },
    methods: {
        translation,
        handleScroll: async function(e) {
            ...
            if (currentPos == maxScroll) {
                const moreWallpapers = (await axios.get(`https://taku.moe:2087/wallpapers/trending/20/${this.wallpapers.length + 1}`)).data.wallpapers;
