...
export default {
  ...
  methods: {   
    ...
    loadCache(){
      ...
      this.privateChannels = channels.filter(channel => channel.type == 'dm' && !channel.isPinned);
      this.groupChannels = channels.filter(channel => channel.type == 'group' && !channel.isPinned);
      this.pinnedChannels = channels.filter(channel => channel.isPinned);
    ...
    async getChannels(){
      ...
      channelsRequest.data.channels.forEach(channel => {
        //console.log(channel); 
        let member_list = [];
        ...
        if (this.channels.some(cachedChannel => cachedChannel.name == channel.name)) return;
