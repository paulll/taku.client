...
app.get('/user', async (req, res) => {
    ...
    jwt.verify(req.cookies.token, "h4x0r", async (error, user) => {
        if (error) {
            res.status(401);
            res.json({"message": "You must be logged in to view your user info"});
            return
        }
        req.user = user;
...
app.get('/message/:user_id', async (req, res) => {
    ...
    const targetUser = (await users.find({"_id": req.params.user_id}, {collation: {locale: 'en',strength: 2}}))[0];
    if (targetUser) {
        dm = (await dms.find({ members: { $in: [targetUser._id, req.user._id]}}))[0];
...
app.post('/login', async (req, res) => {
    ...
    async function checkUser(username, password) {
        const user = await users.find({"username": username}, {collation: {locale: 'en',strength: 2}});
