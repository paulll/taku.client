<template>
  <div class="channel" v-if="channel.member_list[0]" @click="cachedChannels.push(channel.uuid)">
    ...
    <router-link :to="`/messages/${channel.type}/${channel.uuid}`" class="channelContent">
      <router-link :to="`/profile/${channel.member_list[0].username}`" @mouseenter="showMiniProfile($event)" @mouseleave="mouseLeftHover()" ><img class="channelPfp" :src="`http://localhost:2087/pfp/${channel.member_list[0].uuid}`" alt=""></router-link>
      <div class="info">
      <div> 
